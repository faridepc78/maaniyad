@section('admin_title')
    <title>پنل مدیریت مانیاد | مدیریت گالری محصول ({{$product->name}})</title>
@endsection
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('products.index')}}">مدیریت محصول ها</a></li>
                        <li class="breadcrumb-item"><a class="my-active"
                                                       href="{{route('products.gallery.index',$product->id)}}">مدیریت
                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <form id="management_product_gallery_form"
                              action="{{route('products.gallery.store',$product['id'])}}"
                              method="post"
                                ...
                                @if(count($gallery))
                                    ...
                                    @foreach($gallery as $key=>$value)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('products.gallery.destroy', [$product['id'],$value->id]) }}"
                                                   onclick="destroyProductGallery(event,{{ $product['id'] }}, {{ $value->id }})"><i
                                                        class="fa fa-remove text-danger"></i></a>
                                                <form
                                                    action="{{ route('products.gallery.destroy', [$product['id'],$value->id]) }}"
                                                    method="post"
...
<script type="text/javascript">
    ...
    function destroyProductGallery(event,project_id, id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById(`destroy-ProductGallery-${project_id}-${id}`).submit()
            }
        })
    }
